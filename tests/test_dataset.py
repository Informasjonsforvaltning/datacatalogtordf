...
from datacatalogtordf import Dataset, Distribution, Location, PeriodOfTime, URI
...
def test_to_graph_should_return_distribution_as_graph() -> None:
    ...
    distribution1.identifier = URI("http://example.com/distributions/1")
    dataset.distributions.append(distribution1)

    distribution2 = Distribution()
    distribution2.identifier = URI("http://example.com/distributions/2")
...
def test_to_graph_should_return_frequency() -> None:
    ...
    dataset = Dataset()
    dataset.identifier = URI("http://example.com/datasets/1")
    dataset.frequency = URI("http://purl.org/cld/freq/daily")
...
def test_to_graph_should_return_was_generated_by() -> None:
    ...
    dataset = Dataset()
    dataset.identifier = URI("http://example.com/datasets/1")
    dataset.was_generated_by = URI("http://example.com/activity/1")
...
def test_to_graph_should_return_access_rights_comment() -> None:
    ...
    dataset = Dataset()
    dataset.identifier = URI("http://example.com/datasets/1")
    dataset.access_rights_comments.append(URI("http://example.com/concepts/1"))
